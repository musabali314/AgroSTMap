...
# ------------------------------------------------------------
# Guidance primitives (fixed)
# ------------------------------------------------------------
def send_local_position_target(master, n, e, d, yaw_target=None):
    """
    Send position+yaw target (no rotation during flight).
    Ignores velocity, accel, yaw_rate fields.
    """
    mask = (1<<3)|(1<<4)|(1<<5)|(1<<6)|(1<<7)|(1<<8)|(1<<11)  # 0x9F8
    master.mav.set_position_target_local_ned_send(
        0, master.target_system, master.target_component,
        mavutil.mavlink.MAV_FRAME_LOCAL_NED, mask,
        n, e, d,
        0,0,0, 0,0,0,
        0.0 if yaw_target is None else yaw_target,
        0.0
    )

def wait_until_reached(master, n_tgt,e_tgt,d_tgt,n0,e0,d0,yaw0,
                       pos_tol=1.0, timeout_s=25,
                       n_cmd=None, e_cmd=None, d_cmd=None, yaw_cmd=None):
    """Wait until within pos_tol, re-sending yaw-locked setpoint at 10Hz."""
    t0=time.time()
    last_send=0
    while True:
        # Stream setpoint at 10 Hz
        now=time.time()
        if n_cmd is not None and (now - last_send) >= 0.1:
            send_local_position_target(master, n_cmd, e_cmd, d_cmd, yaw_target=yaw_cmd)
            last_send = now
...
def do_custom_path(master, alt_m, n0,e0,d0,yaw0):
    d_target = -alt_m
    yaw_target = yaw0  # lock to takeoff heading

    waypoints_body = [
        (15.0, 0.0, d_target, "Leg 1: forward to (15,0)"),
        (15.0, 2.0, d_target, "Leg 2: strafe right to (15,4)"),
        (0.0,  2.0, d_target, "Leg 3: back left to (0,4)"),
        (0.0,  4.0, d_target, "Leg 4: strafe right to (0,8)"),
        (15.0, 4.0, d_target, "Leg 5: forward to (15,8) — final point")
    ]

    for i,(n_b,e_b,d_t,desc) in enumerate(waypoints_body,1):
        ...
        print(f"[{now_str()}] BODY target ({n_b:+.2f},{e_b:+.2f},{d_t:+.2f}) "
              f"→ WORLD ({n_cmd:+.2f},{e_cmd:+.2f},{d_cmd:+.2f}) | yaw locked {math.degrees(yaw_target):+.1f}°")

        # Send and stream the setpoint with yaw lock
        send_local_position_target(master, n_cmd, e_cmd, d_cmd, yaw_target=yaw_target)
        wait_until_reached(master, n_b, e_b, d_t, n0,e0,d0,yaw0,
                           pos_tol=1.0, timeout_s=25,
                           n_cmd=n_cmd, e_cmd=e_cmd, d_cmd=d_cmd, yaw_cmd=yaw_target)
        time.sleep(1.0)

    # Smooth descent + land at final position
    print(f"[{now_str()}] Descending to ~0.1 m AGL before LAND ...")
    send_local_position_target(master, n_cmd, e_cmd, d0 - 0.1, yaw_target=yaw_target)
    wait_until_reached(master, n_b, e_b, -0.1, n0,e0,d0,yaw0,
                       pos_tol=0.2, timeout_s=10,
                       n_cmd=n_cmd, e_cmd=e_cmd, d_cmd=d0-0.1, yaw_cmd=yaw_target)
...
# ------------------------------------------------------------
def main():
    ap = argparse.ArgumentParser(description="Yaw-locked autonomous path flight (no rotation).")
